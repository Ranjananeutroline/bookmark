...
function AddNoteSection(props) {
  ...
  const creactStickyNoteInDB = (color, stickyNoteDescription) => {
    if (GoogleId) {
      axios
        .post(api+"/stickynote/", {
          GoogleId: GoogleId,
        ...
        .then((res) => {
          props.addNote(
            ...
            res.data.notesId,
            res.data.profileId,
            res.data.stickyNoteDescription
          );
        })
        .catch((err) => {
          console.log(err);
  ...
  const maximizeToDb = (stickyNoteId, profileId) => {
    if (GoogleId) {
      axios
        ...
        <ul className={`color_list ${listOpen ? "color_list_active" : ""}`}>
          {colors.map((item, index) => (
            <li
              ...
              onClick={() => {
                creactStickyNoteInDB(item, "New Note");
              }}
            />
          ))}
        </ul>
                ...
                <div className="innerDiv">
                  <>
                    {props.notes.map((item, index) => {
                      const desc = item.stickyNoteDescription.replace(
                        /(<([^>]+)>)/gi,
                        ""
                      );
                      return (
                        <>
                          ...
                          <div
                            ...
                            onClick={() => {
                              maximizeToDb(item.stickyNoteId, item.ProfileId);
                            }}
                          >
                            {" "}
                            {desc.substring(0, 35)}
